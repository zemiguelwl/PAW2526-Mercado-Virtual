// Sistema de toast para notificações inline (substitui alert())
function showToast(message, type = "info") {
  let container = document.getElementById("posToastContainer");
  if (!container) {
    container = document.createElement("div");
    container.id = "posToastContainer";
    container.style.cssText = "position:fixed;top:16px;right:16px;z-index:9999;display:flex;flex-direction:column;gap:8px;min-width:260px;max-width:380px";
    document.body.appendChild(container);
  }
  const colors = { success: "#198754", error: "#dc3545", info: "#0d6efd", warn: "#fd7e14" };
  const toast = document.createElement("div");
  toast.style.cssText = `background:${colors[type] || colors.info};color:#fff;padding:12px 16px;border-radius:6px;box-shadow:0 2px 8px rgba(0,0,0,.2);font-size:.9rem;display:flex;justify-content:space-between;align-items:center;gap:10px`;
  toast.innerHTML = `<span>${message}</span><button onclick="this.parentElement.remove()" style="background:none;border:none;color:#fff;cursor:pointer;font-size:1.1rem;line-height:1">×</button>`;
...
async function createQuickClient() {
  ...
  try {
    ...
    if (data.success) {
      ...
      showToast(data.message || "Falha ao criar cliente.", "error");
    }
  } catch (err) {
