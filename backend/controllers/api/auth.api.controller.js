...
async function register(req, res, next) {
  try {
    ...
    if (!PHONE_REGEX.test(String(phone).trim())) {
      return res.status(400).json({ message: "Número de telemóvel inválido. Deve ter 9 dígitos e começar por 2, 3 ou 9." });
    }
...
async function login(req, res, next) {
  try {
    ...
    // Apenas clientes podem usar o frontoffice Angular
