...
import { CartService, KG_OPTIONS, LITER_OPTIONS, ProductUnit } from '../../../core/services/cart.service';
...
export class ProductDetailComponent implements OnInit, OnDestroy {
  ...
  get totalPrice(): number {
    ...
    return Math.round(this.product.price * this.selectedQty * 100) / 100;
  }
  ...
  resetQty(): void {
    if (this.unit === 'kg') {
      this.selectedQty = this.kgOptions[0].value;
    } else if (this.unit === 'liter') {
      this.selectedQty = this.literOptions[0].value;
    } else {
      this.selectedQty = 1;
    }
  }
  ...
  incrementQty(): void {
    const next = this.selectedQty + 1;
    if (next <= this.maxQty) this.selectedQty = next;
  }
  ...
  addToCart(): void {
    ...
    const error = this.cartService.addItem(this.product, this.selectedQty);
    if (error) {
